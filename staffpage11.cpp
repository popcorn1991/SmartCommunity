...
StaffPage11::StaffPage11(QWidget *parent) :
    QMainWindow(parent),
    ui(new Ui::StaffPage11)
{
    ui->setupUi(this);
    ...
    //建立并打开数据库
            QSqlDatabase db = QSqlDatabase::addDatabase("QSQLITE","2");
...
StaffPage11::~StaffPage11()
{
    delete ui;
}


void StaffPage11::on_pushButton_2_clicked()
{
    QSqlTableModel *model = qobject_cast<QSqlTableModel*>(ui->tableView->model());
       if (model) {
           QString searchText = ui->lineEdit->text(); // 假设界面上有一个QLineEdit用于输入查询条件
           model->setFilter(QString("Id LIKE '%%1%'").arg(searchText)); // 假设按用户名查询，根据实际表结构调整
           model->select();
       }
}

void StaffPage11::on_pushButton_3_clicked()
{
    QSqlTableModel *model = qobject_cast<QSqlTableModel*>(ui->tableView->model());
        if (model) {
